...
def plot_workload():
    ...
    axes[1][0].set_ylabel("Network Cost ($)", weight='bold', labelpad=30)

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    fig.set_size_inches((14, 8))
    # fig.tight_layout(rect=[0, 0.03, 1, 1], h_pad=0.3, w_pad=0)
    ...
    print("Fig 13 (Full) plotting finished.")
...
def plot_subplot(dataset: pd.DataFrame, ax, topology: str, workload: str,
    ...
    if workload == 'Transformer-17B':
        ax.ticklabel_format(axis='y', style='sci')
        ax.yaxis.major.formatter._useMathText = True
        ax.text(0.15, 0.85, r'($\times 10^{7}$)',
