...
def plot_workload():
    ...
    axes[1][0].set_ylabel("Normalized Perf./Cost", weight='bold', labelpad=7)
    ...
    print("Fig 14 (Full) plotting finished.")
...
def plot_subplot(dataset: pd.DataFrame, ax, topology: str, workload: str,
    ...
    min_value = min(plot_data['PerfCostUp'])
    if min_value < ylim[topology][0]:
        ylim[topology][0] = min_value

    max_value = max(plot_data['PerfCostUp'])
    ...
    # ax.set_title(f"{workload} on {topology_title}", weight='bold')
    ax.set_xlabel("")
    ax.set_ylabel("")
